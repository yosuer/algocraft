...
public class Grafo<E> {
	...
	private String getNodoDistanciaMasCortaEnAnalizados(
			List<String> analizados, Hashtable<String, Integer> distancias) {
	...
	public List<E> getCaminoMinimo(String origen, String destino) {
		...
		while (analizados.size() > 0) {
			String idNodo = getNodoDistanciaMasCortaEnAnalizados(analizados,
					distancias);
			...
			while (i.hasNext()) {
				...
				int distanciaParaNodoVecino = distancias.get(objNodo.getId())
						.intValue();
				if (calculoDistancia < distanciaParaNodoVecino) {
					distancias.put(objNodo.getId(), new Integer(
							calculoDistancia));
